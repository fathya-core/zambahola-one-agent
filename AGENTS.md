...
- **Phase 5 auto (OMAR-PC):** `npm run agent:phase5-auto` — one window only (close old phase4/live-stack windows). Day live + night `omni-train` at `NIGHT_START=20` Riyadh. Logs: `PHASE5-SCHEDULER.jsonl`, `PHASE5-STATE.json` (pushed with telemetry). Missed night: `npm run agent:phase5-night-now`. After manual night train: `npm run agent:phase5-mark-night-done` then restart `agent:phase5-auto` (avoids `night_train_skip in_progress` / duplicate train). If `:8787` down: `npm run agent:phase5-ready` or `agent:phase5-reload`. DL live unlock: `ZAMBAHOLA_DL_LIVE_AUTO=1` when dir. rolling ≥40% for 2h.
