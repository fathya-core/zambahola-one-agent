...
- **Phase 5 auto (OMAR-PC):** `npm run agent:phase5-auto` — one window: day `phase5-ready` + night `omni-train` + telemetry/audit. Optional login task: `agent:phase5-install-task`. Profile: `config/phase5-ready.env`. DL live unlock: `ZAMBAHOLA_DL_LIVE_AUTO=1` when dir. rolling ≥40% for 2h (`data/learning/dl-live-auto.json`).
