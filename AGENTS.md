...
- **Phase 5 auto (OMAR-PC):** `npm run agent:phase5-auto` — one window: day `phase5-ready` + night `omni-train` + telemetry/audit. Schedule: `config/phase5-ready.env` (`NIGHT_START=20`, `DAY_START=6`, Asia/Riyadh). Optional login task: `agent:phase5-install-task`. DL live unlock: `ZAMBAHOLA_DL_LIVE_AUTO=1` when dir. rolling ≥40% for 2h (`data/learning/dl-live-auto.json`).
